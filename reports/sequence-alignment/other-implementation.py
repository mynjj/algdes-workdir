...
for line in sys.stdin:
    if line[0] == '>':
        reading_state = 'SPECIES_NAME'

    if reading_state == 'SPECIES_NAME':
        specie = line.split(' ')[0][1:]
        species_sequence[specie] = ''
        reading_state = 'PROTEIN'

    elif reading_state == 'PROTEIN':
        species_sequence[specie] += line[:-1]

s1 = species_sequence['Human']
s2 = species_sequence['Gorilla']
...
memo = {}
def best_alignment_cost(l1, l2):
    if l1 in memo and l2 in memo[l1]:
        return memo[l1][l2]
    if l1 == 0:
        return delta*l2
    if l2 == 0:
    ...
    # Aligned
    a = get_cost(s1[l1-1], s2[l2-1]) + best_alignment_cost(l1-1, l2-1)
    # l1 aligned with space
    b = delta + best_alignment_cost(l1, l2-1)
    # l2 aligned with space
    c = delta + best_alignment_cost(l1-1, l2)
    if not l1 in memo:
        memo[l1] = {}

    memo[l1][l2] = max(a, b, c)
    return memo[l1][l2]
...
def traceback():
    ...
    while l1 > 0 or l2 > 0:
        ...
        if best_alignment_cost(l1, l2) == get_cost(s1[l1-1], s2[l2-1]) + best_alignment_cost(l1-1, l2-1):
            alignment[0] += s1[l1-1]
            alignment[1] += s2[l2-1]
            l1 -= 1
            l2 -= 1
        elif best_alignment_cost(l1, l2) == delta + best_alignment_cost(l1, l2-1):
            alignment[0] += '-'
            alignment[1] += s2[l2-1]
            l2 -= 1
        elif best_alignment_cost(l1, l2) == delta + best_alignment_cost(l1-1, l2):
            alignment[0] += s1[l1-1]
            alignment[1] += '-'
            l1 -= 1
...
cost = best_alignment_cost(len(s1), len(s2))
print(cost)
print(traceback())
