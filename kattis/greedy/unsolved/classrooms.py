...
"""
Thanks to stanislavkozlovski. https://github.com/stanislavkozlovski/Red-Black-Tree/blob/master/rb_tree.py
...
class RedBlackTree:
    ...
    def remove(self, value):
        ...
        if node_to_remove is None:  # node is not in the tree
            return
        if node_to_remove.count > 1:
            node_to_remove.count -= 1
            self.count -= 1
            return
...
count = 0
occupiedTil = RedBlackTree()
for i in range(0, nCls):
    occupiedTil.add(0)

for start, end in activities:
    # classroom occupied til o_i can be assigned the activity
    # if start >= o_i
    v = occupiedTil.floor(start)
    if v is None:
        continue
    count += 1
    occupiedTil.remove(v)
    occupiedTil.add(end)
print(count)
